...
def load_wos_data_folder(folder_path: str) -> pd.DataFrame:
    """
    批量加载文件夹中所有 Web of Science 纯文本数据
    :param folder_path: 存放所有 txt 的文件夹路径
    :return: 清洗后的合并 DataFrame
    """
    all_records = []
    ...
    # 基础清洗
    if 'DI' in df.columns:
        df = df.drop_duplicates(subset=['DI'])
...
if __name__ == "__main__":
    # ===================== 批量读取整个 data 文件夹 =====================
    data_folder = "../data"  # 批量读取路径
    df = load_wos_data_folder(data_folder)
